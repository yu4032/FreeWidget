...
public class MainHook implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpp) throws Throwable {
        ...
        XposedBridge.log("[FW] init v5-icon-safe");
        ...
        hookProtection(cl);
        XposedBridge.log("[FW] ready v5-icon-safe");
    }
    ...
    private void hookPlacement(ClassLoader cl) {
        try { XposedHelpers.findAndHookMethod("com.miui.home.launcher.compat.LayoutDropRuleForSwapPlaces", cl,
            ...
            new XC_MethodHook(){@Override protected void beforeHookedMethod(MethodHookParam p){
                ...
                // Do not replace MIUI Home's legality decision for icons, folders,
                // or other items. During 4x6 <-> 6x4 conversion DeviceConfig can
                // still describe the source grid; overriding the result globally
                // lets portrait rows 5/6 survive in a 6x4 target layout.
                if (rotating || sx != 4 || sy != 2) return;

                int c=cells("getCellCountX",6), r=cells("getCellCountY",6);
                p.setResult(x>=0&&y>=0&&x+sx<=c&&y+sy<=r);
            }});
        } catch (Throwable e) { XposedBridge.log("[FW] placement: "+e); }
    }
        ...
        try { XposedHelpers.findAndHookMethod(c,cl,"updateCellOccupiedMarks",
            ...
            new XC_MethodHook(){@Override protected void beforeHookedMethod(MethodHookParam p){
                ...
                try{Object info=p.args[1];View view=(View)p.args[0];if(info==null||view==null)return;
                    ...
                    // This module owns only the 4x2 widget correction. Clamping
                    // every ItemInfo here collapses portrait icon rows 5 and 6
                    // onto landscape row 4 before MIUI's native transform runs.
                    if(is4x2(info)){
                        clamp(p.thisObject,view,info,cols,rows,true);
                        if(!rotating)seedPosition(p.thisObject,view,info,cols,rows);
                    }
                }catch(Throwable e){XposedBridge.log("[FW] occupied marks: "+e);}
            }});
        } catch (Throwable e) {}
    ...
    private void protectSave(Object goc) {
        try{int cols=XposedHelpers.getIntField(goc,"mHCells"),rows=XposedHelpers.getIntField(goc,"mVCells");
            ...
            for(Object[]c:occ){if(c==null)continue;for(Object cell:c){if(!(cell instanceof View)||!seen.add(cell))continue;
                ...
                clamp(goc,v,info,cols,rows,false);}}
        }catch(Throwable e){XposedBridge.log("[FW] protectSave: "+e);}
    }
